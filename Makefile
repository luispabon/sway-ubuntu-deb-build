build-everything: build-image json-c-build scdoc-build wlroots-build-deb sway-build-deb swaylock-build-deb swayidle-build-deb swaybg-build-deb xdg-desktop-portal-wlr-build-deb mako-build-deb kanshi-build-deb waybar-build-deb grim-build-deb slurp-build-deb clipman-install
...
sway-build-deb:
	docker run -t --rm -v $(shell pwd):/workdir -w "/workdir" sway_build sh -c 'dpkg -i debs/scdoc_1*deb; dpkg -i debs/libwlroots0_*deb; dpkg -i debs/libwlroots-dev*.deb;dpkg -i debs/libjson-c*.deb; apt-get -f install; cd sway; debuild -b -uc -us'
	make fix-permissions tidy

swaylock-build-deb:
	docker run -t --rm -v $(shell pwd):/workdir -w "/workdir" sway_build sh -c "dpkg -i debs/scdoc_1*deb; dpkg -i debs/libwlroots0_*deb; dpkg -i debs/libwlroots-dev*.deb; cd swaylock; debuild -b -uc -us"
	make fix-permissions tidy

swayidle-build-deb:
	docker run -t --rm -v $(shell pwd):/workdir -w "/workdir" sway_build sh -c "dpkg -i debs/scdoc_1*deb; dpkg -i debs/libwlroots0_*deb; dpkg -i debs/libwlroots-dev*.deb; cd swayidle; debuild -b -uc -us"
	make fix-permissions tidy

swaybg-build-deb:
	docker run -t --rm -v $(shell pwd):/workdir -w "/workdir" sway_build sh -c "dpkg -i debs/scdoc_1*deb; dpkg -i debs/libwlroots0_*deb; dpkg -i debs/libwlroots-dev*.deb; cd swaybg; debuild -b -uc -us"
	make fix-permissions tidy
